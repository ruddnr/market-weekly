...
## 본문 구조
`## Ⅰ. <코너명>` / `## Ⅱ. <코너명>` / `## Ⅲ. <코너명>` / `## Ⅳ. <코너명>` H2 4개 (2026-08-28 개정: Ⅳ = 중장기 코너).
차트는 `charts.deepdive`(주간) · `charts.longview`(약 3년, 글로벌 peer 크로스체크용 지수 포함) ·
`charts.earnings`(분기 실적 — 2026-09-22부터 FactSet 분기 재무, 해외 종목 포함, 막대) · `charts.themes`(Ⅱ 코너 주도 테마 대표 종목 등락, 막대)
네 개를 쓸 수 있다. earnings·themes는 해당 데이터가 없으면 생략한다
(2026-08-28 개정: Ⅳ 보강 구조 — 논지→실적 확인→주가 반영도→다음 확인 이벤트→무효화 조건→리스크 비대칭.
2026-08-31 개정: Ⅱ 코너에 테마 내부 구조·1개월 시세·themes 차트 추가).
